        ...
        <meta
          ...
          content={
            description === null
              ? "Список сайтов " +
                title +
                " КС ГО и Dota 2 с бесплатными скинами и промокодами. Заходи и выбирай лучший " +
                title +
...
export async function getStaticPaths() {
  const res = await axios.get("http://127.0.0.1:1338/api/categories");
...
export async function getStaticProps(ctx) {
  try {
    ...
    const res = await axios
      .all([
        axios.get(
          "http://127.0.0.1:1338/api/categories/?populate=image&filters[slug][$eq]=" +
            slug
        ),
        axios.get(
          "http://127.0.0.1:1338/api/sites-lists?populate=logo&populate=slides&pagination[page]=1&pagination[pageSize]=12&sort[0]=order%3Aasc&filters[$and][0][categories][slug][$eq]=" +
            slug
        ),
        axios.get("http://127.0.0.1:1338/api/categories/"),
      ])
      .then(
        axios.spread((category, sites, categoryAll) => {
    ...
    return {
      props: { ...categoryPage, siteList, slug, catList, categoryPageText },
    };
  } catch (error) {
    return { props: { ...error } };
  }
